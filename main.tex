...
\begin{definition}[Reduction tree]\label{def:reduction tree}
    ...
    \begin{enumerate}
        ...
        \item\label{it:reduction tree loop}
            The branches of the tree are defined by looping cases:
            \begin{enumerate}
                ...
                \item\label{it:reduction tree unfinished}
                    ...
                    \begin{description}
                        \item[\lnot:left]\label{it:reduction tree lnot left}
                        \item[\lnot:right]\label{it:reduction tree lnot right}
                        \item[\land:left]\label{it:reduction tree land left}
                        \item[\land:right]\label{it:reduction tree land right}
                        \item[\lor:left]\label{it:reduction tree lor left}
                        \item[\lor:right]\label{it:reduction tree lor right}
                        \item[\limplies:left]\label{it:reduction tree limplies lef}
                        \item[\limplies:right]\label{it:reduction tree limplies right}
                        \item[\forall:left]\label{it:reduction tree forall left}
                        \item[\forall:right]\label{it:reduction tree forall right}
                        \item[\exists:left]\label{it:reduction tree exists left}
                        \item[\exists:right]\label{it:reduction tree exists right}
                        \item[else]\label{it:reduction tree branch finished}
                            If \(\Pi\) or \(\Lambda\) have \emph{any} formulae in common,
                            do nothing and jump to \ref{it:reduction tree unfinished}.
                            If the sequences \(\Pi\) or \(\Lambda\) have \emph{nothing}
                            in common, rewrite \(\Pi\seq\Lambda\) as a new leaf of this
                            branch and jump to \ref{it:reduction tree unfinished}.
                    \end{description}
            \end{enumerate}
    \end{enumerate}
\end{definition}
