...
\begin{proof}.
    ...
    \('\), \(+\) and \(\times\) in \(s\). We induce in this \(k\).

    In the base case \(k=0\), we only have the single numeral \(\bar0\),
    for which the claim holds: axiomatically \(\seq s=s\), so \(\seq\bar0=\bar0\).
