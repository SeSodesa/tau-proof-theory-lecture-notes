...
\begin{definition}[Proof in \LK]\label{def:proof in LK}
    In classical logic \LK, a \emph{proof} \(P\)
    is a \emph{tree} of sequents, where
    \begin{enumerate}
        \item
            the leaves are axioms \(A\seq A\),
        \item
            the root of \(P\) has no lower sequents (or successors) and
        \item
...
\section{Intuitionistic logic or \LJ, proofs in \LJ}
...
some proofs~\(P\) that are both in \LK{} and \LJ{} are not in \(\LK\).
...
\begin{exercise}[3.9.2]\label{exe:3.9.2}
    Prove in \LJ{:} \(\exists xF(x)\seq\lnot\forall yF(y)\).
\end{exercise}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item
                ...
                with at most \(n = k\) logical symbols. We add one additional allowed connective to these formulae one at at time to close the induction:
                \begin{description}
                    \item[\(\lnot\)]
                    ...
                    \item[\(\limplies\)]
                        ...
                        \begin{equation*}
                            \begin{prooftree}
                                \hypo{\lnot\lnot B\seq\limplies B}
                                \infer1[(inductive hypothesis)]{\seq\lnot\lnot B\limplies B}
                                \hypo{A\seq A}
                                \hypo{B\seq B}
                                \infer2{}
...
\begin{definition}[Starred formula variants in \LK]\label{def:starred formulae in LK}
    Table~\ref{tab:starred variants in LK} defines a mapping,
    between formulae \(A\) and their starred variants \(A^\ast\).

    \begin{tabenv}{Mapping between fomulae \(A\) and their starred transformations \(A^\ast\).}\label{tab:starred variants in LK}
        \begin{tabular}{C|C}
            \toprule
            ...
            B\lor C & \lnot\arcs{\lnot B\land\lnot C} \\
...
\begin{proposition}\label{prop:claim 1 in LK vs LJ}
...
\begin{proposition}\label{prop:claim 2 in LK vs LJ}
    If \(S\) is the sequent \(A_1,\ldots,A_m\seq B_1,\ldots,B_n\),
    then we may define the sequence formula \(S'\coloneqq A_1^\ast,\ldots,A_m^\ast,\lnot B_1^\ast,\ldots,\lnot B^\ast_n\).
...
\begin{proposition}\label{prop:claim 3 in LK vs LJ}
    \(A^\ast\equiv\lnot\lnot A^\ast\) in \LJ.
\end{proposition}

\begin{proof}
    See example~\ref{exa:3.11}.
\end{proof}

\begin{proposition}\label{prop:claim 4 in LK vs LJ}
...
\begin{proof}
    By induction over the number of inferences in \(S\).
\end{proof}
