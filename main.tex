...
\begin{proof}
    Let \(\Gamma\seq\Delta\) be an \emph{unporvable} sequent in \LK{}.
...
Also, if \(R(a_1,\ldots,a_n)\in\cup\Lambda\),
then \( R(a_1,\ldots,a_n)\notin\cup\Gamma\) and
\(\arcs{a_1,\ldots,a_n}\notin\phi(R)\).
...
but none in \(\cup\Lambda\). In the base case \(n=0\),
...
whereas any \(R\arcs{a_1,\ldots,a_n}\in\cup\Lambda\)
...
logical symbols, whereas it does not satisfy any formulae \(B\in\cup\Lambda\)
...
We now produce the formula \(C\in\cup\Lambda\)
by applying \eqref{eq:reduction:lnot:left}.
According to \IH{}, \(\interp\) does not satisfy \(C\in\cup\Lambda\),
meaning \(\lnot C\in\cup\Gamma\) is satisfied.
The case where \(A = \lnot C\in\cup\Lambda\)
...
If \(A = C\land D\in\cup\Lambda\), it will appear on the right
...
Again, this leads to \(C\land D\in\cup\Lambda\) \emph{not} being satisfied
...
If \(A = C\limplies D\in\cup\Lambda\), then by the inductive hypothesis
...
\begin{enumerate}
    \item
        ...
        then \(C\in\cup\Lambda\) is not satisfied in \(\interp\),
...
\(C(a)\) is satisfied by \(\interp\), and therefore
\(\forall xC(x)\in\cup\Gamma\) is satisfied in \(\interp\) by definition.
