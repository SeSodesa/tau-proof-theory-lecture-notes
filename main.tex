...
we can inferred \(S\) from them. This inference could consist of multiple
            ...
            \item\label{implication} Implication:
                \begin{equation}
                    ...
                    \begin{prooftree}
                        \hypo{\Gamma\seq\Delta, C}
                        \hypo{D, \Pi\seq\Lambda}
                        \infer2{C\limplies D, \Gamma, \Pi\limplies\Delta, \Lambda}
                    \end{prooftree}
                \end{equation}
                \begin{equation}
                    \tag{\(\limplies\):right}
...
\end{equation*}
    ...
    Note also that this proof is not possible in \IK.
\end{example}
...
\begin{example}[A cut free proof]\label{exa:cut free proof}
    ...
    \begin{equation*}
        \begin{prooftree}
            ...
            \infer1[\qed]{\seq\arcs{\forall x A(x)\limplies B}\limplies\arcs{\exists x \arcs{A(x)\limplies B}}}
        \end{prooftree}
    \end{equation*}
\end{example}
...
\begin{equation*}
    \begin{prooftree}
        \hypo{T\seq \Delta}
        \infer1[\eqref{eq:land:left}]{A\lor  B, T\seq \Delta}
    \end{prooftree}
    \quad\text{and}\quad
    \begin{prooftree}
        \hypo{T\seq \Delta, A}
...
Intuitionistic logic \IK{} is very similar to classical logic \LK,
