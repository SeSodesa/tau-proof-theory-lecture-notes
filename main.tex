...
\begin{example}[II]\label{exa:PRF sum}
    The addition of natural numbers is a \PRF{}
    of type \eqref{prf:compound recursive}:
    \begin{itemize}
        \item \(f(0,n) = p_2(0,n)\), where \(p_2\)
...
The converse also holds: by lemma \ref{lem:10.5} the consistency of \PA{}
can be expressed by saying that \(\seq\bar0=\bar1\) is not provable in \PA{}.
...
\begin{proof}
    ...
    We make the contrapositie assumption that \(m_1,\ldots,m_n\notin R\).
...
\begin{equation*}
    ...
    \end{prooftree}
\end{equation*}
This makes \PA{} inconsistent, meaning \(R(m_1,\ldots,m_n)\) holds in \(N^n\).
\end{proof}
